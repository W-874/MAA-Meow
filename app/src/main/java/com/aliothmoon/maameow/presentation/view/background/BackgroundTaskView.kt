...
fun BackgroundTaskView(
    ...
    val toolboxDialog = toolboxUiState.dialog
            ...
            Box(
                ...
                if (!state.isFullscreenMonitor) {
                    ...
                    if (isInitialized && canShowTaskActions) {
                        val isToolboxPage = state.current == PanelTab.TOOLS
                        val toolboxShowsStop = toolboxUiState.canStop ||
                            maaState == MaaExecutionState.STOPPING
                        val isStopping = if (isToolboxPage) {
                            toolboxShowsStop
                        } else {
                            maaState == MaaExecutionState.RUNNING ||
                                maaState == MaaExecutionState.STOPPING
                        }
                        val isTransitioning = if (isToolboxPage) {
                            toolboxUiState.isStarting || maaState == MaaExecutionState.STOPPING
                        } else {
                        ...
                        if (!hideToolboxStartForGachaDisclaimer) {
                            Surface(
                                onClick = {
                                    ...
                                    if (isStopping) {
                                        when (state.current) {
                                            ...
                                            PanelTab.TOOLS -> toolboxViewModel.onAction(ToolboxAction.Stop)
                                            else -> Unit
                                        }
                                    } else {
                                        when {
                                            ...
                                            backendBlocked -> Toast.makeText(
                                                ...
                                                PanelTab.TOOLS -> toolboxViewModel.onAction(ToolboxAction.Start)
                                                else -> Unit
                                            }
                                        }
                                    }
